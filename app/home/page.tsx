    ...
    <>
      ...
      <div className="p-3 max-sm:p-0">
        ...
        <div className="flex justify-start items-start max-sm:mt-2">
          <svg onClick={() => setToggleBtn((cur) => !cur)} className={`${toggleBtn ? 'rotate-180 scale-100' : 'rotate-0 scale-80'} transition-all absolute transform duration-500 max-sm:mx-2 ease-in-out w-10 h-10 rounded-full bg-white border-2`} width="35" height="35" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <ul className={`${toggleBtn ? 'pointer-events-auto opacity-100 translate-x-0 scale-95 ' : 'pointer-events-none  opacity-0 translate-x-full scale-110'} transform max-sm:items-center transition-all duration-500 ease-in-out flex justify-start items-center flex-col gap-8 text-base p-5 mt-12 w-60 max-sm:w-full h-96 py-10 px-2 rounded-lg absolute font-pelak bg-white border-2`}>
            <li onClick={() => setActiveTab(<CreateTask handelBack={(e: unknown) => res(e)} />)} className="cursor-pointer">اضافه کردن تسک ✒️</li>
            <li className="cursor-pointer"  onClick={() => setActiveTab(<CreateCategory handelBack={(e:unknown) => res(e)}/>)}>اضافه کردن دسته بندی ✏️</li>
            <li className="cursor-pointer">تسک ها 📒</li>
            <li className="cursor-pointer">دسته بندی ها 📚</li>
            <li className="cursor-pointer">نمودار پیشرفت 📊</li>
            <li className="cursor-pointer">هدف گذاری و رشد 🌱</li>
          </ul>
